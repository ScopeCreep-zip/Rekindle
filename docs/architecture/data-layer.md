...
- **`communities`** — joined communities. Tracks DHT record keys (`manifest_key`,
  ...
  generation, plate-gate `my_segment_index`, `my_subkey_index`, and
  `lamport_clock`. Three columns from the v1.0 coordinator era
  (`coordinator_pseudonym`, `coordinator_route_blob`, `coordinator_epoch`)
  are still present in `001_init.sql` but unread by any code path; they are
  removed in the next schema bump.
