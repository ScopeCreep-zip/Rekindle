...
| B2 | Only `rekindle-transport` (daemon track), `rekindle-protocol` (desktop track), and the `rekindle` Tauri host may import `veilid-core`. `rekindle-transport` additionally depends on `rekindle-protocol` for the shared `veilid_config::build_veilid_config()` builder — a deliberate, unidirectional edge (`rekindle-protocol` must never depend back on `rekindle-transport`). The Tier-1 tuning struct is `rekindle_types::config::VeilidStartupOptions` (no `veilid_core` types). | `cargo xtask check-boundaries` (warn) + `cargo deny [bans]` (pending) | Warn-only. The Tauri-host import predates this wording fix — the rule as previously written was already violated by `src-tauri/Cargo.toml`; long-term goal is still to push host Veilid usage down into the boundary crates |
