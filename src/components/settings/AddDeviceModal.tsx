        ...
        <Show when={pairingQrSvg()}>
          {(svg) => (
            <FormField label="Scan this QR on the new device">
              {/*
                SAFETY (XSS):
                  `svg()` originates from `commands.generatePairingQrSvg()` —
                  a Tauri IPC command implemented in Rust. The SVG is
                  generated locally from the pairing-session bytes; no part
                  of it derives from peer-controlled input.
                  See docs/security/threat-model.md §5b W1 (innerHTML allowed
                  only for SVG generated by trusted Rust IPC).
                  If a future refactor lets attacker-controlled bytes flow
                  into this svg() output, this trust precondition breaks —
                  switch to a vector renderer instead of innerHTML.
                  Semgrep rule `rekindle-no-inner-html` (`.semgrep.yml`)
                  flags every other innerHTML usage; this is the one
                  audited exception.
              */}
              <div
