...
/// P3.3 — dispatcher for the three session-renewal payload variants.
/// Extracted from `handle_incoming_message` so the dispatcher stays
/// under the workspace too_many_lines budget.
fn handle_session_reset_payload(
    app_handle: &tauri::AppHandle,
    state: &Arc<AppState>,
    sender_hex: &str,
    payload: MessagePayload,
...
/// requester's PreKeyBundle in `pending_session_resets` (in-memory only,
/// never persisted before user consent) and emit a notification so the
/// user can confirm via `accept_session_reset` after verifying the
/// safety number out-of-band. Drops requests from non-friends per the
/// strict allowlist.
fn handle_session_reset_request(
    ...
    // Stash the bundle for the user's accept_session_reset command to
    // consume. INSERT-OR-REPLACE: if the peer sent multiple reset
    // requests (e.g., a retry after their UI didn't see our reply), the
    // most recent bundle wins.
    state
...
/// P3.3 — receive a SessionResetAccept from the peer we previously sent
/// a SessionResetRequest to. Installs a fresh responder-side session
/// matching the peer's new initiator-side session. delete_session is
/// idempotent on a missing peer, so repeated accepts (e.g., due to a
/// retry) just overwrite cleanly.
...
///
/// BLAKE3(sort([our_identity_key, peer_identity_key]) || "rekindle-safety-v1")
/// → first 8 hex chars (32 bits = roughly 6 chars-worth of entropy by
/// brute-force; small enough to read aloud, large enough to detect
/// substitution attacks at the cost a casual user would tolerate).
///
...
fn compute_safety_number(
    ...
    let our_x25519_pub = our_identity.to_x25519_public();
    let our_pub_bytes = our_x25519_pub.as_bytes();
