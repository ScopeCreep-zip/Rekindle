...
pub struct AppState {
    ...
    /// Wave 12 W12.12 — peers the user has temporarily muted via the
    /// IncomingCallModal "Mute caller for 1 hour" decline option.
    /// Map: peer_pubkey_hex → expires_at_ms (unix). Incoming offers
    /// from a peer in this map are auto-declined with reason "user is
    /// unavailable" without ringing or persisting a missed-call row,
    /// modeling Discord's "ignore this contact for X" UX. Cleared on
    /// process restart (in-memory only) so a temp-mute never silently
    /// outlives the session it was set in.
    pub temp_call_muted: Arc<Mutex<HashMap<String, u64>>>,

    /// Wave 14 W14.4 — observability counter for voice packets dropped
    /// at the app_message dispatcher (channel full / no active call /
    /// AEAD decrypt fail). Promoted from per-site `tracing::trace!`
    /// drops which were invisible in production. Surfaces every 1 s
    /// via VoiceEvent::PacketsDropped if non-zero.
    pub voice_pkt_drops: std::sync::Arc<std::sync::atomic::AtomicU64>,

    /// Wave 14 W14.1 — pre-staged voice packet receiver. The caller's
    /// CallAccept handler creates the mpsc channel BEFORE deriving the
    /// call_key + calling start_session, then stashes the Receiver
    /// here. spawn_loops takes the receiver from this slot instead of
    /// creating a new channel. The Sender is in `voice_packet_tx` so
    /// the app_message dispatcher can route packets the moment
    /// CallAccept lands — eliminating the lazy-creation race that
    /// dropped the receiver's first ~200ms of audio.
    pub voice_packet_rx_staged: parking_lot::Mutex<
        Option<tokio::sync::mpsc::Receiver<rekindle_voice::transport::VoicePacket>>,
    >,

    /// Wave 12 W12.9 — group call state (1:N), keyed by `call_id`.
    /// Mirrors `active_calls` but holds a participants list instead of
    /// a single peer pubkey. The shared `call_key` is generated by the
    /// initiator and per-participant wrapped via X25519 + HKDF +
    /// AES-256-GCM (`rekindle_calls::group`). Same Drop-zeroize
    /// guarantees as 1:1 because GroupCallState's StaticSecret +
    /// Option<[u8; 32]> both zeroize on drop.
    pub group_calls: Arc<Mutex<HashMap<String, crate::services::group_calls::GroupCallState>>>,
