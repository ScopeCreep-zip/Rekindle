...
// ── Trait-level error conversion ────────────────────────────────────
//
// Maps the exhaustive veilid-specific taxonomy to the transport trait's
// smaller set of variants. Preserves the full error message via Display
// so diagnostics remain actionable after conversion.

impl From<TransportError> for rekindle_types::transport::TransportError {
    fn from(e: TransportError) -> Self {
        use rekindle_types::transport::TransportError as T;
        match e {
            // ── Lifecycle → StartFailed / NotAttached ───────────
            TransportError::NotStarted
            | TransportError::NetworkNotReady => T::NotAttached,

            TransportError::AlreadyStarted
            | TransportError::AttachFailed { .. }
            | TransportError::ShutdownFailed { .. } => T::StartFailed(e.to_string()),

            // ── Routing → RouteFailed ───────────────────────────
            TransportError::NoRoute { .. }
            | TransportError::RouteImportFailed { .. }
            | TransportError::RouteExpired { .. }
            | TransportError::CircuitOpen { .. }
            | TransportError::RouteAllocationFailed { .. } => T::RouteFailed {
                reason: e.to_string(),
            },

            // ── Send → SendFailed / Timeout ─────────────────────
            TransportError::PayloadTooLarge { .. }
            | TransportError::SendFailed { .. } => T::SendFailed {
                reason: e.to_string(),
            },

            TransportError::Timeout { .. } => T::Timeout { seconds: 0 },

            // ── DHT → RecordFailed / RecordNotFound ─────────────
            TransportError::RecordNotOpen { ref key } => T::RecordNotFound {
                key: key.clone(),
            },

            TransportError::RecordCreateFailed { .. }
            | TransportError::SubkeyTooLarge { .. }
            | TransportError::StaleWrite { .. }
            | TransportError::WatchDied { .. }
            | TransportError::DhtError { .. } => T::RecordFailed {
                reason: e.to_string(),
            },

            // ── Serialization ───────────────────────────────────
            TransportError::SerializationFailed { reason } => T::Serialization(reason),
            TransportError::DeserializationFailed { reason, .. } => T::Deserialization(reason),

            // ── Everything else → Internal ──────────────────────
            _ => T::Internal(e.to_string()),
        }
    }
}
