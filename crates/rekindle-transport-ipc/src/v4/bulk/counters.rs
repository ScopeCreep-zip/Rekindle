...
/// audit_tx channel at zero capacity when a lane task attempted .send().
/// Non-zero means audit_merge is falling behind lane task throughput.
/// Under sustained contention (8×4K120fps + bulk), investigate tokio
/// task scheduling — audit_merge may be starved.
pub static DIAG_AUDIT_TX_BACKPRESSURE: AtomicU64 = AtomicU64::new(0);
/// Bulk decrypt chunks dropped because early_bulk_chunks buffer was full
/// (64 entries). STREAM_OPEN never arrived before 64 chunks accumulated.
/// Non-zero indicates protocol ordering violation or extremely large
/// burst-before-open. Investigate chunk_size vs reassembler creation timing.
pub static DIAG_EARLY_BULK_CHUNKS_DROPPED: AtomicU64 = AtomicU64::new(0);
...
pub fn reset_all_diagnostics() {
    ...
    DIAG_AUDIT_TX_BACKPRESSURE.store(0, Ordering::Relaxed);
    DIAG_EARLY_BULK_CHUNKS_DROPPED.store(0, Ordering::Relaxed);
}
