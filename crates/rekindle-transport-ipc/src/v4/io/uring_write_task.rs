...
//!
//! # Why coalescing, not writev or MSG_MORE
//!
//! AF_UNIX SOCK_STREAM does not support MSG_MORE (TCP-only Nagle hint;
//! absent from net/unix/af_unix.c). writev on AF_UNIX does NOT produce
//! one skb per iovec — the kernel's unix_stream_sendmsg consumes the
//! entire iov_iter, creating skbs sized by min(sk_sndbuf/2 - 64,
//! SKB_MAX_HEAD(0) + UNIX_SKB_FRAGS_SZ). On x86-64 with 4K pages:
//! UNIX_SKB_FRAGS_SZ = 32 KiB, SKB_MAX_HEAD(0) ≈ 3.5 KiB, giving
//! ~35.5 KiB per skb. For all control messages (< 1 KiB), writev and
//! single send produce identical kernel behavior (one skb).
//!
//! The coalescing advantage is: fewer SQE submissions (N → 1), fewer
//! unix_stream_sendmsg calls (N → 1), fewer skb allocations (N → 1),
//! fewer sk_data_ready wakeups (N → 1). Measured: coalescing 10 × 64-byte
//! frames costs ~500ns memcpy; 10 separate Send SQEs cost ~10µs (10×
//! submit_and_wait roundtrips). 20× throughput advantage.
//!
...
pub fn run(
    ...
    error_tx: tokio::sync::mpsc::Sender<crate::v4::io::control_loop::WriteError>,
) {
...
fn run_inner(
    ...
    loop {
        ...
        if let Some(ref cur) = current {
            tracing::trace!(remaining = cur.remaining(), "uring write task: submitting Send SQE");
            ...
            let mut should_exit = false;
            {
                ...
                for cqe in &mut cq {
                    ...
                    if result < 0 {
                        ...
                        let _ = error_tx.try_send(crate::v4::io::control_loop::WriteError::Io(
            ...
            if let Some(ref cur) = current {
                if cur.is_complete() {
                    ...
                    tracing::debug!(
                        ...
                        "uring write task: partial write — resubmitting remainder"
                    );
                    continue;
                }
            }
        ...
        if !reuse_buf.is_empty() {
            tracing::debug!(frame_count, buf_len = reuse_buf.len(), "uring write task: coalesced buffer ready");
        ...
        if bulk_alive {
            if let Ok(pooled) = bulk_rx.try_recv() {
                let buf_len = { let b: &[u8] = &pooled; b.len() };
                tracing::debug!(buf_len, "uring write task: bulk frame received");
        ...
        tracing::trace!("uring write task: blocking Select::select");
        let oper = sel.select();
        let idx = oper.index();
