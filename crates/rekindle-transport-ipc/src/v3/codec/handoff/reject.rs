...
pub struct HandoffRejectPayload {
    pub reason_code: FailureCode,
    pub handoff_id: uuid::Uuid,
    pub detail: String,
}

pub fn encode(p: &HandoffRejectPayload) -> Vec<u8> {
    ...
    crate::v3::codec::write_u32_len(&mut buf, detail_bytes.len());
    buf.extend_from_slice(p.handoff_id.as_bytes());                    // 8..24
    buf.extend_from_slice(detail_bytes);                               // 24..
    buf
}

pub fn decode(buf: &[u8]) -> Result<HandoffRejectPayload, CodecError> {
    if buf.len() < 24 {
    ...
    Ok(HandoffRejectPayload { reason_code, handoff_id, detail })
}
