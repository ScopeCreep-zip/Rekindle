...
//! SessionContext, does NOT dispatch application frames, does NOT run timers.
...
pub async fn run(
    ...
    loop {
        ...
        if env_info.lane == Lane::Data && env_info.body_len >= bulk_threshold {
            ...
            bulk_receiver.dispatch_from_parts(env_info.session_seq, env_info.key_epoch(), &envelope_buf, &body);
            continue;
        }
