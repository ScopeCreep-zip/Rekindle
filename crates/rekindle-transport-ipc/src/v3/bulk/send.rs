...
use crate::v3::codec::stream::open as open_codec;
use crate::v3::context::{OutboundFrame, PendingFin, SequencedOutbound};
use crate::v3::io::encode::FrameEncoder;
use crate::v3::io::lane_channels::{BulkFrame, WireBufPool};
...
impl BulkSender {
    ...
    pub async fn send(
        ...
        // Merkle content hash — computed on the tokio thread before dispatch.
        // Required because STREAM_OPEN carries content_hash and must be sent
        // before any payload chunks. BLAKE3 at ~10 GiB/s makes this ~100µs
        // per MiB — acceptable for the send path's latency budget.
        let mut content_hasher = blake3::Hasher::new();
        for (i, chunk) in payload.chunks(chunk_size).enumerate() {
            let digest = *blake3::hash(chunk).as_bytes();
            tracing::debug!(
                stream_id, chunk_index = i, chunk_len = chunk.len(),
        ...
        for (i, chunk) in payload.chunks(chunk_size).enumerate() {
            ...
            let header_flags = if chunk_count == 1 && is_last_chunk {
                crate::v3::wire::header::flags::FIN_FOLLOWS
            } else {
                0
            };
            ...
            self.encrypt_pool.spawn(move || {
                ...
                // Read AAD directly from wire_buf — envelope and header are
                // already at known offsets. No redundant captured allocations.
                ...
                crate::v3::bulk::counters::DIAG_AUDIT_OUTBOUND_PUSHES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                if chunk_audit_queue.try_push(seq, link_input).is_err() {
                    crate::v3::bulk::counters::DIAG_AUDIT_OUTBOUND_PUSH_FAILURES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    tracing::error!(stream_id, chunk = cidx, "audit queue full — dropping chunk, session will terminate");
                    return;
                }

                if chunk_wire_tx.try_send(BulkFrame::Pooled(wire_buf)).is_err() {
                    tracing::error!(stream_id, chunk = cidx, "bulk_wire_tx full or closed — write task dead");
                    return;
                }

                tracing::debug!(stream_id, chunk = cidx, seq, "rayon: wire + audit sent");
            });
        }
        ...
        if chunk_count > 1 {
            ...
            tracing::debug!(stream_id, chunk_count, "bulk_send: PendingFin sent");
        } else {
