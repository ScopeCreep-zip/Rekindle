...
async fn test_ergonomic_errors() -> Result<()> {
    ...
    // A unit-returning call succeeds — the `?` is the assertion (the original
    // `let result = ...; assert!(result == ())` tripped clippy's
    // let_unit_value + unit_cmp; the explicit error type keeps inference
    // unambiguous now that the binding is gone).
    conn.call(|_conn| Ok::<_, rusqlite::Error>(())).await?;
