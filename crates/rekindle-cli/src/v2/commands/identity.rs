...
pub async fn cmd_init(args: &InitArgs, client: &DaemonClient, mode: OutputMode) -> anyhow::Result<()> {
    if args.wipe_all_data {
        ...
        let value = client.request_ok(DaemonRequest::IdentityWipe {
    ...
    let existing = client.request_ok(DaemonRequest::IdentityShow).await;
    if existing.is_ok() {
        format::step_skip("Identity already exists")?;
        format::step_header(1, 2, "Unlocking daemon")?;
        let _ = client.request_ok(DaemonRequest::Unlock { passphrase: String::new() }).await?;
    ...
    let value = client.request_ok(DaemonRequest::IdentityCreate { display_name: display_name.clone() }).await?;
    ...
    if let Some(ref export_path) = args.export_identity {
        let export_value = client.request_ok(DaemonRequest::IdentityExport).await?;
...
pub async fn dispatch(cmd: &IdentityCmd, client: &DaemonClient, mode: OutputMode) -> anyhow::Result<()> {
    match cmd {
        IdentityCmd::Show { .. } => {
            let value = client.request_ok(DaemonRequest::IdentityShow).await?;
            format::print_structured(&value, mode)
        }
        IdentityCmd::Rotate { force } => {
            ...
            let value = client.request_ok(DaemonRequest::IdentityRotate).await?;
        ...
        IdentityCmd::Destroy => {
            ...
            if !confirmed { return format::print_text("Cancelled."); }
            let value = client.request_ok(DaemonRequest::IdentityDestroy {
        ...
        IdentityCmd::Export { path, passphrase } => {
            if *passphrase {
                ...
                // The moved String lives only in the DaemonRequest until serialization.
                let passphrase_owned = std::mem::take(&mut *pass);
                let value = client.request_ok(DaemonRequest::IdentityExportEncrypted {
                ...
                let value = client.request_ok(DaemonRequest::IdentityExport).await?;
        ...
        IdentityCmd::Import { path, passphrase } => {
            ...
            if *passphrase {
                ...
                let _value = client.request_ok(DaemonRequest::IdentityImportEncrypted {
                    passphrase: passphrase_owned,
                    data: blob_b64,
                }).await?;
                ...
                let _value = client.request_ok(DaemonRequest::IdentityImport {
                    data: json,
                }).await?;
                format::print_text("Identity imported successfully.")
            }
