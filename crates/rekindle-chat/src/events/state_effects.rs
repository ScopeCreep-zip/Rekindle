...
//!
//! Called once per event after construction, before dedup + emit.
//! Updates unread counts, typing indicators, presence, voice state.
//! Returns additional events to emit (e.g., `UnreadChanged` after
//! incrementing a counter).
//!
//! Pure state mutation — no I/O, no network calls, no logging.
...
pub fn apply(state: &mut SubscriptionState, event: &SubscriptionEvent) -> Vec<SubscriptionEvent> {
    ...
    match event {
        ...
        SubscriptionEvent::Voice(VoiceEvent::Joined { community, channel, pseudonym }) => {
            ...
            state.voice.join(community, channel, pseudonym, now);
        }
