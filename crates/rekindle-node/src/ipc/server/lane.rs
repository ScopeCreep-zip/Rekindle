...
//!
//! Lane values match `bulk::FrameKind` for bulk frames:
...
///
/// Returns `Ok(lane)` on success, `Err` on I/O error or EOF.
/// EOF is reported as `UnexpectedEof` — the caller should treat
/// it as a clean disconnect.
...
/// Write a lane byte to the writer.
///
/// Does NOT flush — the caller batches the lane byte with the
/// subsequent frame data before flushing.
...
/// Returns true if the lane byte indicates a bulk-plane frame.
#[inline]
