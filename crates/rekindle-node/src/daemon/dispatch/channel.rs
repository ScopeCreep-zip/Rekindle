...
pub(crate) async fn handle_create(
    ...
    let name = match validation::validate_name(name, "Channel") { Ok(n) => n, Err(e) => return e };
    if let Err(e) = validation::validate_channel_kind(kind) { return e; }
    let transport = match ctx.require_transport() { Ok(t) => t, Err(e) => return e };
    let membership = match ctx.resolve_community(community) { Ok(m) => m, Err(e) => return e };

    match rekindle_transport::operations::channel_admin::create_channel(
        &transport, &membership.governance_key, &name, kind, category, topic, slowmode_seconds,
    ).await {
...
pub(crate) async fn handle_delete(
    ...
    match rekindle_transport::operations::channel_admin::delete_channel(
        &transport, &membership.governance_key, channel_id,
    ).await {
...
pub(crate) async fn handle_update(
    ...
    if !state.can_write() { return state_error(state, "write"); }
    if let Some(n) = name {
        if let Err(e) = validation::validate_name(n, "Channel") { return e; }
    }
    let transport = match ctx.require_transport() { Ok(t) => t, Err(e) => return e };
    let membership = match ctx.resolve_community(community) { Ok(m) => m, Err(e) => return e };

    match rekindle_transport::operations::channel_admin::update_channel(
        &transport, &membership.governance_key, channel_id, name, topic, slowmode_seconds,
    ).await {
        Ok(entry) => IpcResponse::ok(&serde_json::json!({
            "id": entry.id,
            "name": entry.name,
...
pub(crate) async fn handle_send(
    ...
    let detail = match query.community_detail(&membership).await {
        Ok(d) => d, Err(e) => return IpcResponse::error(500, format!("community detail: {e}")),
    };
    let Some(ch) = detail.channels.iter().find(|c| c.name == channel || c.id == channel) else {
    ...
    let existing_log_keypair_bytes = if let Some(existing_key) = membership.channel_record_keys.get(&channel_id) {
        let short = if existing_key.len() > 12 { &existing_key[..12] } else { existing_key };
    ...
    match rekindle_transport::operations::channel::send_message(
        &transport, &membership, &channel_id, body, reply_to, &ctx.mek_cache,
            ...
            if is_new_log {
                {
                    ...
                    if let Some(ref mut session) = *guard {
                        if let Some(m) = session.communities.get_mut(&membership.governance_key) {
                            m.channel_record_keys.insert(channel_id.clone(), sent.member_record_key.clone());
                        }
                    }
                }
                ...
                if membership.is_operator {
                    ...
                    if let Ok(transport_clone) = ctx.require_transport() {
                        if let Ok(dht) = transport_clone.dht() {
                            crate::daemon::community_rpc::open_registry_writable(&transport_clone, &membership.registry_key).await;
                            let mut members = dht.registry().read_member_index(&membership.registry_key).await.unwrap_or_default();
                            if let Some(m) = members.iter_mut().find(|m| m.pseudonym_key == membership.pseudonym_key) {
                                m.channel_records.insert(channel_id.clone(), sent.member_record_key.clone());
                                let _ = dht.registry().write_member_index(&membership.registry_key, &members).await;
                    ...
                    if let Ok(transport_clone) = ctx.require_transport() {
                        if let Ok(signing_key) = ctx.require_signing_key() {
                            if !membership.community_mailbox_key.is_empty() {
                                ...
                                if let Ok(dht) = transport_clone.dht() {
                                    if let Ok(Some(route_blob)) = dht.mailbox().read_community_route(&mailbox_key).await {
                                        if let Ok(target) = transport_clone.import_route(&route_blob) {
                                            ...
                                            let op_bytes = postcard::to_stdvec(&op).unwrap_or_default();
...
pub(crate) async fn handle_history(
    ...
    let channel_id = detail.channels.iter()
        .find(|ch| ch.name == channel || ch.id == channel)
        .map_or(channel, |ch| ch.id.as_str());
    match query.channel_history(
        &membership.governance_key, channel_id, "", &membership.registry_key, limit as usize,
