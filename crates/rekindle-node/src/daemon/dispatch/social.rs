...
pub(crate) async fn handle_friend_add(
    ctx: &DaemonContext, state: DaemonState, target: &str, message: &str,
) -> IpcResponse {
    if !state.can_write() { return state_error(state, "write"); }
    if let Err(e) = validation::validate_key(target, "target mailbox key") { return e; }
...
pub(crate) async fn handle_friend_accept(
    ctx: &DaemonContext, state: DaemonState, public_key: &str,
) -> IpcResponse {
    if !state.can_write() { return state_error(state, "write"); }
    let transport = match ctx.require_transport() { Ok(t) => t, Err(e) => return e };
    ...
    let (session, request) = {
        ...
        if let Some(result) = found {
            ...
            let inbox_key = ctx.session.read().as_ref()
            ...
            if let Some(ref key) = inbox_key {
                ...
                while scan_start.elapsed() < scan_deadline {
                    crate::daemon::friend_inbox::scan_friend_inbox(
                        &ctx.session, &ctx.transport, &ctx.session_path, key,
                    ).await;
    ...
    match rekindle_transport::operations::friend::accept_friend_request(
        ...
        Ok(accepted) => {
            ...
            let log_short = if accepted.dm_log_key.len() > 12 { &accepted.dm_log_key[..12] } else { &accepted.dm_log_key };
            let label = format!("dm-log-{log_short}");
            let _ = crate::state::keystore::store_keypair_bytes(&label, &accepted.dm_log_keypair_bytes).await;
            ...
            // Update session: remove pending request, add dm_log_key mapping
            {
                ...
                if let Some(ref mut s) = *guard {
                    ...
                    s.dm_log_keys.insert(public_key.to_string(), accepted.dm_log_key.clone());
                }
            }
...
pub(crate) async fn handle_dm_send(
    ctx: &DaemonContext, state: DaemonState, peer_key: &str, body: &str,
) -> IpcResponse {
    if !state.can_write() { return state_error(state, "write"); }
    ...
    let dm_log_keypair_bytes = if let Some(log_key) = session.dm_log_keys.get(peer_key) {
        let short = if log_key.len() > 12 { &log_key[..12] } else { log_key };
    ...
    match rekindle_transport::operations::dm::send_dm(
        &transport, &session, peer_key, &mailbox_key, body, &signing_key, dm_log_keypair_bytes.as_deref(),
    ).await {
...
pub(crate) async fn handle_dm_typing(
    ctx: &DaemonContext, state: DaemonState, peer_key: &str, typing: bool,
) -> IpcResponse {
    if !state.can_write() { return state_error(state, "write"); }
    let transport = match ctx.require_transport() { Ok(t) => t, Err(e) => return e };
    let signing_key = match ctx.require_signing_key() { Ok(k) => k, Err(e) => return e };
...
pub(crate) async fn handle_dm_inbox(
    ctx: &DaemonContext, state: DaemonState, limit: u32,
) -> IpcResponse {
    if !state.can_query() { return state_error(state, "query"); }
    let transport = match ctx.require_transport() { Ok(t) => t, Err(e) => return e };
    ...
    for (peer_key, log_key) in &dm_log_keys {
        match query.dm_inbox(log_key, &friend_list_key, limit as usize, &our_key).await {
