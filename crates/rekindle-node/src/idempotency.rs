...
impl IdempotencyCache {
    ...
    pub fn check(&self, client_msg_id: &str) -> Option<DaemonResponse> {
    ...
    pub fn store(&self, client_msg_id: String, response: DaemonResponse) {
...
mod tests {
    ...
    fn miss_then_hit() {
        ...
        cache.store("req-1".into(), DaemonResponse::ok(&serde_json::json!({"sent": true})));
    ...
    fn different_ids_independent() {
        let cache = IdempotencyCache::new();
        cache.store("a".into(), DaemonResponse::ok(&serde_json::json!(1)));
        cache.store("b".into(), DaemonResponse::ok(&serde_json::json!(2)));
    ...
    fn capacity_eviction() {
        ...
        for i in 0..CACHE_CAPACITY + 100 {
            cache.store(format!("req-{i}"), DaemonResponse::ok(&serde_json::json!(i)));
        }
