...
impl VaultStore {
    ...
    pub fn store_sent_dm(
        ...
        let conn = self.conn();
        conn.execute(
            "INSERT OR IGNORE INTO dm_sent (peer_key, sender_key, body, timestamp, message_id)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![peer_key, sender_key, ct, i64::try_from(timestamp).unwrap_or(i64::MAX), message_id],
        )?;
        Ok(())
    }
    ...
    pub fn query_sent_dm(&self, peer_key: &str, limit: u32) -> StorageResult<Vec<DmRecord>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT body, timestamp, message_id, sender_key FROM dm_sent
