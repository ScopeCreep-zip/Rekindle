//! Universal SMPL/DFLT schema builders — the Q-pid equation.
//!
...
//!
//! These builders live here (a `veilid-core` boundary crate) rather than in
//! `rekindle-records` so that records stays free of `veilid-core` (Invariant 2).
//! `rekindle-records::schema` keeps only the pure `MAX_MEMBERS_PER_SEGMENT`.
...
/// Build a DFLT schema for a single invite's encrypted `InviteSecrets` record.
///
/// One owner-written subkey holding the opaque encrypted blob. Lives in its
/// own record (not the size-capped governance SMPL subkey) so the several-KB
/// invite payload fits the larger single-owner budget; governance carries only
/// a pointer to this key. See architecture §6 (invite distribution).
pub fn invite_secrets_dflt_schema() -> VeilidAPIResult<DHTSchema> {
    DHTSchema::dflt(1)
}
...
mod tests {
    ...
    fn community_schema_creates_with_ocnt_zero() {
        ...
        match schema {
            ...
            DHTSchema::DFLT(_) => panic!("expected SMPL schema"),
        }
    }
    ...
    fn bootstrap_schema_is_dflt() {
        ...
        match schema {
            ...
            DHTSchema::SMPL(_) => panic!("expected DFLT schema"),
        }
    }
}
