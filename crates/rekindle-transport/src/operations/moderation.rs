...
pub fn build_kick_payload(target_pseudonym: &str) -> Result<Vec<u8>> {
    serialize_control(ControlPayload::Kick {
...
pub async fn ban_member(
    ...
    if bans.iter().any(|b| b.pseudonym_key == target_pseudonym) {
        return Err(TransportError::DhtError {
            reason: format!("{target_pseudonym} already banned"),
        });
    }
    bans.push(BanEntry {
    ...
    serialize_control(ControlPayload::Ban {
        target_pseudonym: target_pseudonym.to_string(),
    })
}
...
pub async fn unban_member(
    ...
    if bans.len() == before {
        return Err(TransportError::DhtError {
            reason: format!("{target_pseudonym} not banned"),
        });
    }
    dht.governance().write_bans(governance_key, &bans).await?;
    info!(target = target_pseudonym, "member unbanned");
    serialize_control(ControlPayload::Unban {
        target_pseudonym: target_pseudonym.to_string(),
    })
}
...
fn serialize_control(ctrl: ControlPayload) -> Result<Vec<u8>> {
    postcard::to_stdvec(&GossipPayload::Control(ctrl)).map_err(|e| {
