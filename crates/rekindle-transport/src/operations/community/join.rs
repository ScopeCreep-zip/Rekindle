//! Join flow — split into three phases for event-driven completion:
//! - [`submit_join_request`] — writes to inbox, returns immediately
//! - [`await_join_approval`] — select! between registry poll (tier 3)
//!   and direct notification (tier 2)
//! - [`complete_join`] — reads channels + MEKs after approval confirmed
//!
//! [`join_community`] is a convenience wrapper that calls all three
//! sequentially.
...
use super::{pseudonym_to_inbox_subkey, JoinRequestSubmitted, JoinResult};
...
pub async fn complete_join(
    ...
    for entry in &vault {
            ...
            .find(|c| c.target_pseudonym == submitted.our_pseudonym_hex)
        {
            ...
            match crate::operations::mek::receive_mek_transfer_payload(
                &transfer,
                signing_key_bytes,
