...
pub async fn send_message(
    ...
    plaintext: &str, reply_to_sequence: Option<u64>,
    mek_cache: &Arc<RwLock<MekCache>>, existing_log_keypair_bytes: Option<&[u8]>,
) -> Result<MessageSent> {
    ...
    let mek_generation = mek_cache.read().current(&membership.governance_key, channel_id).map_or(0, Mek::generation);

    // Step 2: Build message
    let message_id = uuid::Uuid::new_v4().to_string();
    let timestamp = rekindle_utils::timestamp_ms();
    let channel_msg = ChannelMessage {
        sequence: 0, sender_pseudonym: membership.pseudonym_key.clone(),
        ciphertext, mek_generation, timestamp, reply_to: reply_to_sequence,
        lamport_ts: timestamp, message_id: Some(message_id.clone()),
    };
    let msg_bytes = serde_json::to_vec(&channel_msg)
    ...
    let (log, spine_key) = if let (Some(key), Some(kp)) =
        (membership.channel_record_keys.get(channel_id), existing_log_keypair)
    {
    ...
    Ok(MessageSent { message_id, sequence, timestamp, member_record_key: spine_key, new_log_keypair_bytes })
}
