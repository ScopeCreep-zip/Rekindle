...
use veilid_core::{DHTSchema, KeyPair, RoutingContext, CRYPTO_KIND_VLD0};
...
impl ShortArray {
    ...
    pub async fn create(
        ...
        let total = capacity
            ...
            .ok_or_else(|| TransportError::RecordCreateFailed {
                reason: "capacity overflow".into(),
            })?;

        let schema = DHTSchema::dflt(total).map_err(|e| TransportError::RecordCreateFailed {
            reason: format!("{e}"),
        })?;
        ...
        let head_bytes =
            postcard::to_stdvec(&head).map_err(|e| TransportError::SerializationFailed {
                reason: e.to_string(),
            })?;
        rc.set_dht_value(key.clone(), 0, head_bytes, None)
            .await
            .map_err(|e| TransportError::DhtError {
                reason: format!("write head: {e}"),
            })?;
    ...
    pub async fn open(rc: &RoutingContext, key: &str, writer: Option<KeyPair>) -> Result<Self> {
        ...
        let _ =
            rc.open_dht_record(rk.clone(), writer)
                .await
                .map_err(|e| TransportError::DhtError {
                    reason: format!("open short array: {e}"),
                })?;
    ...
    async fn write_head(&self, head: &ShortArrayHead) -> Result<()> {
        let bytes = postcard::to_stdvec(head).map_err(|e| TransportError::SerializationFailed {
            reason: e.to_string(),
        })?;
        self.rc
...
async fn read_head_raw(
...
) -> Result<ShortArrayHead> {
    let value =
        rc.get_dht_value(key.clone(), 0, false)
            .await
            .map_err(|e| TransportError::DhtError {
    ...
    match value {
        Some(v) => {
            postcard::from_bytes(v.data()).map_err(|e| TransportError::DeserializationFailed {
                type_id: 0,
                reason: format!("head: {e}"),
            })
        }
        None => Err(TransportError::DhtError {
            reason: "head subkey not set".into(),
        }),
    }
}
