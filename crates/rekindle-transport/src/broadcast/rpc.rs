...
use crate::payload::rpc::{
    CallResponse, CommunityLeaveNotification, GovernanceOp,
    GovernanceOpResponse, GovernanceRequest, SyncRequest, SyncResponse,
};
...
pub async fn community_leave(
    ...
    debug!(governance = governance_key, pseudonym = leaving_pseudonym, "rpc: community_leave");
    ...
    let payload = postcard::to_stdvec(&notification)
        .map_err(|e| TransportError::SerializationFailed { reason: e.to_string() })?;

    let response_bytes = node.caller().call(
        target, TypeId::CommunityLeave, signing_key, sender_public_hex, &payload,
    ).await?;

    let response: CallResponse = postcard::from_bytes(&response_bytes)
        .unwrap_or(CallResponse::Ack);
...
pub async fn governance_op(
    ...
    let response_bytes = node.caller().call(
        target, TypeId::CommunityGovOp, signing_key, sender_public_hex, &payload,
    ).await?;

    let response: GovernanceOpResponse = postcard::from_bytes(&response_bytes)
        .unwrap_or(GovernanceOpResponse::Failed {
...
pub async fn governance_kick(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    target_pseudonym: &str, signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
        GovernanceOp::Kick { target_pseudonym: target_pseudonym.into() },
        signing_key, sender_hex).await
}

/// Convenience: send a Ban governance op.
pub async fn governance_ban(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    target_pseudonym: &str, reason: Option<&str>,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
        GovernanceOp::Ban {
...
pub async fn governance_unban(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::Unban { target_pseudonym: target_pseudonym.into() },
        signing_key, sender_hex).await
}
...
pub async fn governance_timeout(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    target_pseudonym: &str, duration_secs: u64, reason: Option<&str>,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
...
pub async fn governance_create_channel(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    name: &str, kind: &str, topic: Option<&str>,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
        GovernanceOp::CreateChannel {
            name: name.into(), kind: kind.into(),
...
pub async fn governance_delete_channel(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    channel_id: &str, signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
        GovernanceOp::DeleteChannel { channel_id: channel_id.into() },
        signing_key, sender_hex).await
}
...
pub async fn governance_update_channel(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    channel_id: &str, name: Option<&str>, topic: Option<&str>,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
...
pub async fn governance_create_role(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::CreateRole { name: name.into(), permissions, color, position },
        signing_key, sender_hex).await
}
...
pub async fn governance_update_role(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::UpdateRole {
            role_id, name: name.map(String::from), permissions, color,
        },
        signing_key, sender_hex).await
}
...
pub async fn governance_delete_role(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    role_id: u32, signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
...
pub async fn governance_assign_role(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::AssignRole { member_pseudonym: member_pseudonym.into(), role_id },
        signing_key, sender_hex).await
}
...
pub async fn governance_unassign_role(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::UnassignRole { member_pseudonym: member_pseudonym.into(), role_id },
        signing_key, sender_hex).await
}
...
pub async fn governance_rotate_mek(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::RotateMek { channel_id: channel_id.into() },
        signing_key, sender_hex).await
}
...
pub async fn governance_transfer_ownership(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    new_owner_pseudonym: &str, signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
        GovernanceOp::TransferOwnership { new_owner_pseudonym: new_owner_pseudonym.into() },
        signing_key, sender_hex).await
}
...
pub async fn governance_register_channel_record(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    member_pseudonym: &str, channel_id: &str, record_key: &str,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
...
pub async fn governance_approve_join(
    ...
    governance_op(node, target, governance_key,
        GovernanceOp::ApproveJoin { target_pseudonym: target_pseudonym.into() },
        signing_key, sender_hex).await
}
...
pub async fn governance_reject_join(
    node: &TransportNode, target: &PeerTarget, governance_key: &str,
    target_pseudonym: &str, reason: &str,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<GovernanceOpResponse> {
    governance_op(node, target, governance_key,
...
pub async fn sync_request(
    node: &TransportNode, target: &PeerTarget,
    channel_id: &str, since_timestamp: u64,
    signing_key: &[u8; 32], sender_hex: &str,
) -> Result<SyncResponse> {
    debug!(channel = channel_id, since = since_timestamp, "rpc: sync_request");
    ...
    let response_bytes = node.caller().call_with_timeout(
        target, TypeId::SyncRequest, signing_key, sender_hex, &payload, SYNC_TIMEOUT,
    ).await?;

    let response: SyncResponse = postcard::from_bytes(&response_bytes)
        .map_err(|e| TransportError::DeserializationFailed {
            type_id: TypeId::SyncResponse as u8,
            reason: e.to_string(),
        })?;

    info!(channel = channel_id, messages = response.messages.len(), "sync response received");
    Ok(response)
}
