//! Peer-to-peer DM broadcast — Signal-encrypted DhtLog writes + ephemeral signals.
//!
//! DM message content: Signal-encrypted ciphertext written to the sender's
//! outbound DhtLog. No app_message fallback — DMs are persistent and offline-safe.
//!
//! Ephemeral signals (typing, ack, unfriend, presence): sent via app_message
//! to the peer's route. These are real-time, lost if peer offline, and do not
...
/// Write a Signal-encrypted DM to the sender's outbound DhtLog.
///
/// `body` is Signal ciphertext bytes — the caller (handle_dm_send in
/// the node crate) performs Signal encrypt before calling this.
/// The ciphertext is hex-encoded in the DhtLog JSON entry.
///
/// There is no app_message fallback. DMs are persistent and offline-safe
/// exclusively through DhtLog. If no outbound log exists, the friendship
/// is not established and DMs are refused.
pub async fn direct_message(
    node: &TransportNode, session: &Session,
    peer_key: &str, body: &[u8], _reply_to: Option<Vec<u8>>,
    _signing_key: &[u8; 32], dm_log_keypair: Option<veilid_core::KeyPair>,
) -> Result<()> {
    debug!(peer = &peer_key[..12.min(peer_key.len())], body_bytes = body.len(), "dm: direct_message");

    let (Some(peer_log), Some(kp)) = (session.dm_peers.get(peer_key), dm_log_keypair) else {
        return Err(TransportError::NoRoute {
            peer: format!("{}… — no outbound DM log (friendship not established)", &peer_key[..12.min(peer_key.len())]),
        });
    };
    let log_key = &peer_log.outbound_log_key;
    if log_key.is_empty() {
        return Err(TransportError::NoRoute {
            peer: format!("{}… — outbound DM log not established", &peer_key[..12.min(peer_key.len())]),
        });
    }
    let dht = node.dht()?;
...
// friend_accept and friend_reject wrappers removed.
// The daemon calls operations::friend::* directly from dispatch/social.rs.
