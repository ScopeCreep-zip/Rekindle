...
use crate::payload::gossip::{
    ControlPayload, CommunityEvent, GameServerInfo, GossipPayload,
    OnboardingAnswer, ThreadInfo, VoiceParticipant,
};
...
pub async fn member_joined(
    ...
    build_sign_send(node, meshes, community_id, sender_pseudonym, signing_key, payload).await
}
...
async fn build_sign_send(
    ...
    sender.broadcast_gossip(&targets_with_routes, &envelope).await
}
...
pub async fn send_direct(
    ...
    let envelope = envelope::sign_gossip_envelope(
        &signing, community_id, sender_pseudonym,
        &payload_bytes, 0, 0, // TTL=0 (no forwarding), lamport=0 (single-shot)
    );
