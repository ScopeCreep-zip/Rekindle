...
impl<'a> ProfileOps<'a> {
    ...
    pub async fn create(
        ...
        let (key, keypair) = record::create_dflt(
            self.rc,
            PROFILE_SUBKEY_COUNT as u16,
            None,
        ).await?;

        record::set(self.rc, &key, PROFILE_SUBKEY_DISPLAY_NAME, display_name.as_bytes().to_vec(), None).await?;
        record::set(self.rc, &key, PROFILE_SUBKEY_STATUS_MESSAGE, status_message.as_bytes().to_vec(), None).await?;
    ...
    pub async fn open_or_create(
        ...
        if let (Some(key), Some(kp)) = (existing_key, &existing_keypair) {
            match self.try_reopen_and_update(
                key, kp.clone(), display_name, status_message, prekey_bundle, route_blob,
            ).await {
                Ok(()) => {
        ...
        let (key, keypair) = self.create(display_name, status_message, prekey_bundle, route_blob).await?;
        Ok((key, keypair, true))
    }
