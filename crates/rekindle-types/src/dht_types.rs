...
pub struct MemberSummary {
    ...
    /// Member's X25519 DH public key (hex-encoded, 64 chars).
    ///
    /// Published alongside the Ed25519 pseudonym so the community operator
    /// can ECDH-wrap MEKs for this member during key rotation. Derived from
    /// the member's pseudonym seed via `BLAKE3 derive_key("rekindle identity
    /// x25519 v1", &pseudonym_seed)` → X25519 keypair → public key.
    #[serde(default)]
...
pub struct PendingJoinEntry {
    ...
    /// X25519 DH public key (hex-encoded, 64 chars). Published so the
    /// community operator can ECDH-wrap MEKs for this member on approval.
    #[serde(default)]
...
pub struct FriendRequestEntry {
    ...
    pub sent_at: u64,
    /// Sender's X25519 DH public key (hex-encoded, 64 chars).
    /// Derived deterministically from the sender's identity seed.
    /// Used by the responder for PQXDH handshake and future ECDH operations.
    #[serde(default)]
    pub x25519_pub_hex: String,
...
impl FriendRequestEntry {
    ...
    ///
    /// Covers identity-binding fields + PQXDH handshake material.
    /// For Accepted entries, the pqxdh_init_message blob is included
    /// so an attacker cannot swap handshake material on the world-writable
    /// DHT inbox without invalidating the signature.
    pub fn signature_content(&self) -> Vec<u8> {
        let mut content = Vec::new();
        ...
        // Include PQXDH init message in signature for Accepted entries.
        // Prevents an attacker from replacing handshake material while
        // preserving a valid signature from a legitimate Pending entry.
        if let FriendRequestStatus::Accepted { ref pqxdh_init_message, .. } = self.status {
...
pub enum FriendRequestStatus {
    ...
    /// responder's messages). Also contains the PQXDH init message as
    /// a serialized blob so the sender can complete `pqxdh::respond()`
    /// and derive the same session key.
    ///
    /// The blob approach (vs individual fields) means:
    /// - One serde_json::to_vec in accept.rs, one from_slice in respond.rs
    /// - PqxdhInitMessage is the single source of truth for field names/types
    /// - Future PQXDH field additions (SPQR epoch, braid state) don't change this enum
    Accepted {
        ...
        /// JSON-serialized `PqxdhInitMessage` from `rekindle_ratchet::pqxdh::bundle`.
        ///
        /// Contains: ik_a_x25519, ik_a_ed25519, ek_a, spk_b_id, pqpk_b_id,
        /// pqpk_is_last_resort, opk_b_id, kem_ct (1088 bytes), initial_ciphertext.
        ///
        /// The sender deserializes this blob and passes it to `pqxdh::respond()`
        /// to complete the handshake. Maximum expected size ~4KB JSON.
        /// Reject blobs > 8192 bytes before deserialization.
